      ...
      public async Task UpdateAppointmentSlotStatus(Guid appointmentdSlotId, AppointmentSlotStatus status)
        {
           RabbitMqPublisher rabbitMQPublisher = new();
            await rabbitMQPublisher.PublishMessageAsync<AppointmentSlotStatusUpdated>(RabbitMQQueues.AppointmentStatusUpdated, new() { AppointmentSlotId = appointmentdSlotId, Status = status.ToString() });
